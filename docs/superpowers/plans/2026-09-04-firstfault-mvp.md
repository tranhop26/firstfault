...
- Missing, stale, unavailable, malformed, contradictory, or insufficient evidence cannot cause payout or refund; it yields `REQUEST_MORE_INFO` or `UNRESOLVED`.
...
- [ ] **Step 5: Validate results deterministically**
  ...
  Reject unknown outcomes, invalid/duplicate step indexes, missing citations, `FIRST_BREACH` without one breached step, contradictory statuses, and any model-generated address or amount. Source failure, low confidence, or consensus insufficiency stores `UNRESOLVED` and schedules no transfer.
