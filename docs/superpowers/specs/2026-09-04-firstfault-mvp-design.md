...
- validator disagreement → transaction rollback → `DISPUTED`; only after the 3,600-second expiry from `dispute_opened_at` and all stored observations are strictly stale, `timeout_dispute_to_unresolved → UNRESOLVED`
