...
Extend only the historical reconciliation matcher. It will recognize these three representations:
...
Every representation must match both an allow-listed settlement method and the exact first workflow argument. The implementation must not use a general substring match, must not infer success from UI or local storage, and must keep the existing finalized/execution-success and child-allocation checks unchanged.
...
| RPC/history provider | A merely present transaction | Return unrelated, failed, or incomplete history entries | Preserve contract address, type, `FINALIZED`, execution-success, triggered-child, and exact allocation checks | Existing reconciliation tests plus live-chain readback |
...
Keep the change local to the calldata matcher in `frontend/lib/contracts/FirstFault.ts`. Add the smallest exact parser branch for the current representation. Avoid a broad normalization pass because malformed calldata must fail closed.

Add a regression test to `frontend/lib/contracts/FirstFault.test.ts` using the exact live decoded calldata shape. The test must fail before the production change and pass afterward. A negative case must prove that a different first workflow argument is rejected even if the target workflow appears elsewhere.
...
3. Run the complete non-Localnet frontend suite, TypeScript check, and production build.
4. Call `findSettlementEvidence` against the live Studio Next contract and require the known parent plus all three known child hashes.
5. After separately confirmed GitHub/Vercel actions, verify a fresh production browser displays Explorer links for the parent and three payouts.
