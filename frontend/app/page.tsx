            ...
            <div className="ff-workspace">
              ...
              <aside className="ff-side-column">
                <section className="ff-panel ff-action-card"><span className="ff-eyebrow">Available actions</span><h2>Case controls</h2><p>Buttons reflect your connected role; the contract enforces final authorization.</p>
                  {app.workflow.state === 'DRAFT' && isBuyer && <button disabled={!writable} className="ff-button ff-button-primary ff-full" onClick={() => app.fund(total)}>Fund {formatGen(total)} simulated GEN</button>}
                  ...
                  {app.contractVersion === 'v2' && app.workflow.state === 'IN_PROGRESS' && (workerDeadlineExpired ? <button disabled={!writable} className="ff-button ff-button-outline ff-full" onClick={app.timeoutIncomplete}>Open adjudication for missed delivery</button> : <div className="ff-no-action">Recovery becomes available after the current worker deadline.</div>)}
                  {app.contractVersion === 'v2' && app.workflow.state === 'READY_FOR_REVIEW' && (reviewDeadlineExpired ? <button disabled={!writable} className="ff-button ff-button-outline ff-full" onClick={app.timeoutReview}>Open adjudication after review silence</button> : <div className="ff-no-action">The buyer review window is still open.</div>)}
                  {app.contractVersion === 'v2' && app.workflow.state === 'UNRESOLVED' && ['ADJUDICATION_UNRESOLVED', 'CONSENSUS_TIMEOUT'].includes(app.workflow.unresolved_reason ?? '') && (retryReady ? <button disabled={!writable} className="ff-button ff-button-outline ff-full" onClick={app.retryAdjudication}>Retry the same evidence</button> : <div className="ff-no-action">The one-hour consensus retry delay is still running.</div>)}
